...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "properties" | "users" | "dealer_approvals"
  >("dashboard");
        ...
        (payload) => {
          ...
          if (updatedProfile.approval_status !== "pending") {
            ...
            setStats((prevStats) => ({
              ...prevStats,
              pendingDealers:
                prevStats.pendingDealers > 0 ? prevStats.pendingDealers - 1 : 0,
            }));
          }
        },
      )
  ...
  const fetchData = async () => {
    if (activeTab === "dashboard") {
      ...
      await fetchUsers();
    } else if (activeTab === "dealer_approvals") {
      // New tab for dealer approvals
  ...
  const fetchProperties = async () => {
    ...
    if (error) {
      ...
      console.log(
        `✓ Successfully fetched ${data.length} ${propertyFilter} properties`,
      );
  ...
  const updateDealerApprovalStatus = async (
    dealerId: string,
    status: "approved" | "rejected",
  ) => {
    const supabase = createClient();
    ...
    if (status === "rejected") {
      ...
      alert(
        `Dealer approved successfully! They can now sign in and create listings.`,
      );
    }
  ...
  const deleteUser = async (userId: string, userName: string) => {
    if (
      !confirm(
        `Are you sure you want to delete user "${userName}"? This will also delete all their properties and data.`,
      )
    )
      return;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-[#444444] mb-2">
          Admin Panel
        </h1>
        <p className="text-sm sm:text-base text-[#767676]">
          Manage your LegacyEstate platform
        </p>
      </div>
      ...
      {activeTab === "dashboard" && (
        <div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-[#c1bfbf]/30 p-6 hover:shadow-md transition-shadow duration-200">
              <div className="flex items-center">
                <div className="flex-1">
                  ...
                  <p className="text-2xl sm:text-3xl font-bold text-[#444444]">
                    {stats.totalProperties}
                  </p>
                </div>
                <div className="bg-[#33a137]/10 rounded-full p-3">
                  <svg
                    className="w-8 h-8 text-[#33a137]"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            <div className="bg-white rounded-lg shadow-sm border border-[#c1bfbf]/30 p-6 hover:shadow-md transition-shadow duration-200">
              <div className="flex items-center">
                <div className="flex-1">
                  ...
                  <p className="text-2xl sm:text-3xl font-bold text-[#33a137]">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-[#c1bfbf]/30 p-6">
            <h2 className="text-xl font-semibold text-[#444444] mb-4">
              Quick Actions
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                <h3 className="font-semibold text-[#444444]">
                  Review Pending Dealers
                </h3>
                <p className="text-sm text-[#767676] mt-1">
                  {stats.pendingDealers} dealers awaiting approval
                </p>
              </button>
              <button
                onClick={() => setActiveTab("users")}
          ...
          {fetchingProperties ? (
            <div className="bg-white rounded-lg shadow-sm border border-[#c1bfbf]/30 p-12 text-center">
              ...
              <p className="text-[#767676]">
                Loading {propertyFilter} properties...
              </p>
            </div>
          ) : properties.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border border-[#c1bfbf]/30 p-12 text-center">
              <p className="text-[#444444] text-lg font-medium mb-2">
                No {propertyFilter === "all" ? "" : propertyFilter} properties
                found
              </p>
              <p className="text-[#767676] text-sm">
                Try clicking the Refresh button or check a different filter
              </p>
            </div>
          ) : (
            <>
              <div className="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-blue-800 font-semibold">
                  ...
                  {propertyFilter === "all" ? "" : propertyFilter}{" "}
          ...
          {pendingDealers.length === 0 ? (
            <div className="bg-white rounded-lg shadow-md p-12 text-center">
              ...
              <p className="text-gray-500 text-sm">
                All dealers are currently approved or none have applied.
              </p>
            </div>
          ) : (
                    ...
                    {users.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() =>
                              deleteUser(user.id, user.full_name || "Unknown")
                            }
                            className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 text-sm"
                          >
                            Delete
